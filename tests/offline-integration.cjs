...
function runOfflineCoreSmoke() {
  const source = `
    ...
    global.chrome={runtime:{getURL:path=>'chrome-extension://test/'+path,onMessage:{addListener(){}}},storage:{local:{
      ...
      set:async value=>{savedSettings=value.settings}
    }}};
    ...
    (async()=>{
      ...
      if(chain.chain!=='FRONTEND_EDGE_AZURE')throw new Error('Edge route mismatch');
      ...
      if(savedSettings?.translateEngine!=='google'||savedSettings?.voicesType!=='free'||savedSettings?.toLanguage!=='vi')throw new Error('settings migration mismatch');
...
async function testEdgeTts() {
  ...
  const ssml = `<speak version="1.0" xml:lang="vi-VN"><voice name="vi-VN-HoaiMyNeural">Xin chào, đây là bài kiểm tra lồng tiếng.</voice></speak>`;
  ...
  if (!audioResponse.ok) {
    throw new Error(`Edge TTS returned HTTP ${audioResponse.status}: ${await audioResponse.text()}`);
  }
  const audio = await audioResponse.arrayBuffer();
  assert(audio.byteLength > 1000, `Edge TTS audio is too small: ${audio.byteLength}`);
  assert(/audio/i.test(audioResponse.headers.get("content-type") || ""), "Edge TTS content type is not audio");
  return { bytes: audio.byteLength, contentType: audioResponse.headers.get("content-type") };
}
